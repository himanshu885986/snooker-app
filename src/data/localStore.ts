...
// The real database stores them hashed (supabase/schema.sql).

import { billableMinutes, billableSeconds, frameAmount, splitAmount, visitTotals } from '../lib/billing'
import { assertCan, isValidPin, normalizePhone, type Action } from '../lib/permissions'
import type {
  Account, AuthApi, Branch, BranchState, Charge, DataStore, Frame, FramePause, FramePlayer, Member,
  NewPlayer, Org, PaymentMode, Payment, Product, Role, Side, Table, Visit,
} from './types'
  ...
  return {
    ...
    openVisit(branchId, player: NewPlayer) {
      return write('operate', (db, at) => {
        ...
        const id = newId()
        db.visits.push({ id, branch_id: branchId, player_name: name, phone: player.phone?.trim() || null, opened_at: at, closed_at: null, status: 'open' })
        return id
      })
    },
    ...
    checkout(visitId, mode: PaymentMode) {
      return write('collect', (db, at) => {
        const visit = db.visits.find((v) => v.id === visitId)
        if (!visit || visit.status !== 'open') throw new Error('Player has already checked out')
        if (isPlaying(db, visitId)) throw new Error(`${visit.player_name} is still in a running frame. End the frame first.`)
        const { due } = visitTotals({
          charges: db.charges.filter((c) => c.visit_id === visitId),
          payments: db.payments.filter((p) => p.visit_id === visitId),
        })
        if (due > 0) db.payments.push({ id: newId(), visit_id: visitId, amount_paise: due, mode, created_at: at })
